...
for i in `docker ps | awk '{print $NF}' | grep "^centos7"`; do
  killContainer "$i"
done

for i in `docker ps -a | awk '{print $NF}' | grep "^centos7"`; do
  rmContainer "$i"
done

for i in $(seq 1 $NUM_TARGETS); do
  if [ "$NON_ROOT_AGENT" == "yes" ]; then
    exitOnFail docker run -d --net ambari --hostname "centos7$i.ambari" --name "centos7$i" -e YUM_PROXY=http://squid:3128 non-root-ambari-agent "$PUB_KEY" ambari
  else
    exitOnFail docker run -d --net ambari --hostname "centos7$i.ambari" --name "centos7$i" -e YUM_PROXY=http://squid:3128 root-ambari-agent "$PUB_KEY" ambari
  fi
  echo "Target node with hostname centos7$i created"
done
